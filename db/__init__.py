...
cur.execute('''CREATE TABLE IF NOT EXISTS users
               (id INTEGER PRIMARY KEY, name TEXT, email TEXT UNIQUE, password TEXT, role INT DEFAULT 0, quota REAL,
               login_provider TEXT)''')
...
cur.execute('''CREATE TABLE IF NOT EXISTS job_files
               (id INTEGER PRIMARY KEY, filepath TEXT, filament_length REAL, user_id int,
               FOREIGN KEY (user_id) REFERENCES users(id))''')

cur.execute('''CREATE TABLE IF NOT EXISTS job_history
               (id INTEGER PRIMARY KEY, time_started REAL DEFAULT CURRENT_TIMESTAMP, time_finished REAL,
